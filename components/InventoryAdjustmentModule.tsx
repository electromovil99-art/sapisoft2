...
const InventoryAdjustmentModule: React.FC<InventoryAdjustmentProps> = ({ 
    ...
    const [barcodeApiSupported, setBarcodeApiSupported] = useState(true);
    ...
    // Limpieza de cámara al desmontar para liberar hardware
    useEffect(() => {
        return () => {
            stopCameraScanner();
        };
    ...
    // Lógica de Cámara Scanner Mejorada
    const startCameraScanner = async () => {
        if (isCameraLoading) return;
        
        setIsCameraLoading(true);
        const isSupported = 'BarcodeDetector' in window;
        setBarcodeApiSupported(isSupported);
        
        try {
            // Asegurar limpieza de cualquier intento previo
            stopCameraScanner();

            // Solicitar acceso a cámara con parámetros para códigos de barras
            ...
            // Breve espera para que el elemento video esté disponible en el DOM
            setTimeout(async () => {
                if (videoRef.current) {
                    videoRef.current.srcObject = stream;
                    try {
                        // Play explícito para iniciar la captura visual
                        await videoRef.current.play();
                    } catch (playError) {
                        console.error("Fallo al reproducir video:", playError);
                    }
                    ...
                    if (isSupported) {
                        ...
                        // Intervalo de escaneo rápido para mejor respuesta
            ...
            // Manejo específico de Permisos Denegados o Ignorados (Permission dismissed)
            if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError' || err.message?.includes('dismissed')) {
                alert("Permiso de cámara denegado. Para usar el escáner, haga clic en el candado de la barra de direcciones, habilite la cámara y recargue la página.");
            } else {
    ...
    const handleDetectedCode = (code: string) => {
        ...
        if (product) {
            ...
            console.log("Código detectado pero no registrado en sistema:", code);
        }
    };
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                    ...
                    <div className="grid grid-cols-2 md:flex gap-2 w-full md:w-auto">
                        ...
                        <button onClick={() => setScanMode(!scanMode)} className={`col-span-2 md:col-auto px-4 py-2 md:py-1.5 rounded-xl text-[9px] md:text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all shadow-sm ${scanMode ? 'bg-emerald-600 text-white animate-pulse' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400'}`}>
                            <ScanBarcode size={14}/> {scanMode ? 'Escáner Activo' : 'Activar Escáner'}
                        </button>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[2000] flex flex-col animate-in fade-in duration-300">
                    <div className="p-4 flex justify-between items-center bg-black/50 shrink-0 border-b border-white/10">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-white font-black text-xs uppercase tracking-widest">Escáner de Código de Barras</h3>
                        </div>
                    ...
                    <div className="flex-1 relative flex items-center justify-center overflow-hidden">
                        ...
                        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                            <div className="w-72 h-48 border-2 border-primary-500 rounded-3xl relative shadow-[0_0_0_2000px_rgba(0,0,0,0.6)]">
                                <div className="absolute -top-1 -left-1 w-8 h-8 border-t-4 border-l-4 border-primary-400 rounded-tl-xl"></div>
                                <div className="absolute -top-1 -right-1 w-8 h-8 border-t-4 border-r-4 border-primary-400 rounded-tr-xl"></div>
                                <div className="absolute -bottom-1 -left-1 w-8 h-8 border-b-4 border-l-4 border-primary-400 rounded-bl-xl"></div>
                                <div className="absolute -bottom-1 -right-1 w-8 h-8 border-b-4 border-r-4 border-primary-400 rounded-br-xl"></div>
                                
                                {/* Línea de escaneo láser */}
                                <div className="absolute top-0 left-0 w-full h-0.5 bg-primary-500/50 shadow-[0_0_15px_rgba(147,51,234,0.8)] animate-[scan_2s_linear_infinite]"></div>
                            </div>
                        </div>

                        {!barcodeApiSupported && (
                             <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-amber-500/90 text-white px-6 py-3 rounded-2xl flex items-center gap-3 shadow-2xl border border-amber-400/50 animate-bounce">
                                 <AlertCircle size={24}/>
                                 <div className="text-left">
                                     <p className="font-black text-xs uppercase">Escáner No Soportado</p>
                                     <p className="text-[10px] font-bold opacity-90">Use SKU manual o instale Google Chrome.</p>
                                 </div>
                             </div>
                        )}
                    </div>
                    ...
                    <div className="p-8 bg-black/50 text-center shrink-0">
                        <p className="text-white/60 text-[10px] font-bold uppercase tracking-[0.2em]">Encuadre el código de barras dentro del visor</p>
                        <p className="text-primary-400 text-[8px] mt-2 font-black uppercase tracking-widest">SapiSoft AI Scan Active</p>
                    </div>
