                    ...
                    {quotations.map(q => (
                        <div key={q.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 flex justify-between items-center group">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <button onClick={() => onLoadQuotation(q)} className="px-4 py-2 bg-blue-600 text-white font-bold rounded-lg text-sm flex items-center gap-2 hover:bg-blue-700">
                                    <Upload size={16}/> Cargar Venta
                                </button>
                                <button onClick={() => onDeleteQuotation(q.id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors opacity-0 group-hover:opacity-100">
                                    <Trash2 size={16}/>
                                </button>
                            </div>
                        </div>
