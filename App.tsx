...
const App: React.FC = () => {
  ...
  const handleProcessInventorySession = useCallback((sessionData: InventoryHistorySession) => {
    ...
    if (sessionData.status === 'ADJUSTED') {
        const newMovements: StockMovement[] = sessionData.items.map(item => ({
            ...
            date, time, productId: item.productId, productName: item.productName, type: item.difference > 0 ? 'ENTRADA' : 'SALIDA', quantity: Math.abs(item.difference), currentStock: item.physicalCount, reference: `AJUSTE FÍSICO FINAL #${sessionData.id}`, user: sessionData.user
        }));
  ...
  const handleAddProduct = useCallback((p: Product) => {
    ...
    const move: StockMovement = {
        ...
        date: new Date().toLocaleDateString('es-PE'), time: new Date().toLocaleTimeString('es-PE'),
        productId: p.id, productName: p.name, type: 'ENTRADA', quantity: p.stock, currentStock: p.stock, reference: 'INVENTARIO INICIAL', user: session?.user.fullName || 'Admin'
    };
    setStockMovements(prev => [move, ...prev]);
  }, [session]);

  const handleUpdateProduct = useCallback((p: Product) => setProducts(prev => prev.map(x => x.id === p.id ? p : x)), []);
  const handleDeleteProduct = useCallback((id: string) => setProducts(prev => prev.filter(x => x.id !== id)), []);
  const handleAddCashMovement = useCallback((m: CashMovement) => setCashMovements(prev => [m, ...prev]), []);
  ...
  const handleUniversalTransfer = useCallback((fromId: string, toId: string, amount: number, exchangeRate: number, reference: string) => {
      ...
      const egress: CashMovement = { id: `TR-OUT-${transferId}`, date, time, type: 'Egreso', paymentMethod: fromId === 'CASH' ? 'Efectivo' : 'Transferencia', concept: `TRANSFERENCIA A ${toId === 'CASH' ? 'CAJA' : 'BANCO'} - REF: ${reference}`, amount, user, category: 'TRANSFERENCIA', financialType: 'Variable', accountId: fromId === 'CASH' ? undefined : fromId };
      const ingress: CashMovement = { id: `TR-IN-${transferId}`, date, time, type: 'Ingreso', paymentMethod: toId === 'CASH' ? 'Efectivo' : 'Transferencia', concept: `TRANSFERENCIA DE ${fromId === 'CASH' ? 'CAJA' : 'BANCO'} - REF: ${reference}`, amount: amount * (exchangeRate || 1), user, category: 'TRANSFERENCIA', financialType: 'Variable', accountId: toId === 'CASH' ? undefined : toId };
  ...
  const renderCurrentView = () => {
    ...
    switch (currentView) {
      case ViewState.SUPER_ADMIN_DASHBOARD: return <SuperAdminModule tenants={tenants} onAddTenant={(t, u) => { setTenants([...tenants, t]); setSystemUsers([...systemUsers, u]); }} onUpdateTenant={(id, updates, newPass) => { setTenants(tenants.map(t => t.id === id ? { ...t, ...updates } : t)); if (newPass) { setSystemUsers(systemUsers.map(u => { const tenant = tenants.find(ten => ten.id === id); if (u.companyName === tenant?.companyName && u.role === 'ADMIN') return { ...u, password: newPass }; return u; })); } }} onDeleteTenant={id => setTenants(tenants.filter(t => t.id !== id))} onResetTenantData={id => alert("Datos restablecidos.")} />;
      case ViewState.POS: return <SalesModule products={products} clients={clients} categories={categories} purchasesHistory={purchasesHistory} bankAccounts={bankAccounts} locations={locations} onAddClient={c => setClients([...clients, c])} onProcessSale={(cart, total, docType, clientName, breakdown, ticketId, detailedPayments, currency, exchangeRate) => { const date = new Date().toLocaleDateString('es-PE'); const time = new Date().toLocaleTimeString('es-PE'); const user = session.user.fullName; const newSale: SaleRecord = { id: ticketId, date, time, clientName, docType, total, items: [...cart], paymentBreakdown: breakdown, user, currency: currency as any, exchangeRate }; setSalesHistory(prev => [newSale, ...prev]); cart.forEach(item => handleManualAdjustment(item.id, 'SALIDA', item.quantity, `VENTA ${docType} #${ticketId}`)); detailedPayments.forEach(p => handleAddCashMovement({ id: Math.random().toString(), date, time, type: 'Ingreso', paymentMethod: p.method, concept: `VENTA ${docType} #${ticketId}`, user, category: 'VENTA', financialType: 'Variable', amount: p.amount, accountId: p.accountId, referenceId: ticketId, currency: currency })); }} cart={posCart} setCart={setPosCart} client={posClient} setClient={setPosClient} quotations={quotations} onLoadQuotation={q => { setPosCart([...q.items]); setPosClient(clients.find(c => c.name === q.clientName) || null); setQuotations(prev => prev.filter(x => x.id !== q.id)); }} onAddQuotation={q => setQuotations(prev => [q, ...prev])} systemBaseCurrency={session.baseCurrency} />;
      case ViewState.SERVICES: return <ServicesModule services={services} products={products} categories={categories} bankAccounts={bankAccounts} onAddService={s => setServices([s, ...services])} onFinalizeService={(id, total, status, breakdown) => setServices(prev => prev.map(s => s.id === id ? { ...s, status } : s))} onMarkRepaired={id => setServices(prev => prev.map(s => s.id === id ? { ...s, status: 'Reparado' } : s))} clients={clients} locations={locations} onOpenWhatsApp={(n, p, m) => { setWsContact({ name: n, phone: p, message: m }); setCurrentView(ViewState.WHATSAPP); }} />;
      case ViewState.PURCHASES: return <PurchaseModule products={products} suppliers={suppliers} categories={categories} bankAccounts={bankAccounts} onAddSupplier={s => setSuppliers([...suppliers, s])} locations={locations} onProcessPurchase={(cart, total, docType, supplierName, condition, creditDays, detailedPayments, currency, exchangeRate) => { const date = new Date().toLocaleDateString('es-PE'); const time = new Date().toLocaleTimeString('es-PE'); const user = session.user.fullName; const purchaseId = 'PUR-' + Math.floor(Math.random() * 1000000).toString(); const newPurchase: PurchaseRecord = { id: purchaseId, date, time, supplierName, docType, total, items: [...cart], paymentCondition: condition, user, currency: currency as any, exchangeRate: exchangeRate || 1 }; setPurchasesHistory(prev => [newPurchase, ...prev]); cart.forEach(item => handleManualAdjustment(item.id, 'ENTRADA', item.quantity, `COMPRA ${docType} #${purchaseId}`)); if (condition === 'Contado' && detailedPayments) { detailedPayments.forEach(pay => handleAddCashMovement({ id: Math.random().toString(), date, time, type: 'Egreso', paymentMethod: pay.method, concept: `COMPRA ${docType} #${purchaseId} - ${supplierName}`, user, category: 'COMPRA', financialType: 'Variable', amount: pay.amount, accountId: pay.accountId, referenceId: purchaseId, currency: currency })); } }} systemBaseCurrency={session.baseCurrency} />;
      ...
      case ViewState.CASH: return <CashModule movements={cashMovements} salesHistory={salesHistory} onAddMovement={handleAddCashMovement} bankAccounts={bankAccounts} onUniversalTransfer={handleUniversalTransfer} fixedExpenseCategories={FIXED_EXPENSE_CATEGORIES} fixedIncomeCategories={FIXED_INCOME_CATEGORIES} onAddFixedCategory={() => {}} isCashBoxOpen={isCashBoxOpen} lastClosingCash={lastClosingCash} onOpenCashBox={(cash, notes, banks) => setIsCashBoxOpen(true)} onCloseCashBox={(c, s, d, n, b) => setIsCashBoxOpen(false)} systemBaseCurrency={session.baseCurrency} />;
      ...
      case ViewState.FIXED_EXPENSES: return <FinanceManagerModule activeTab="EXPENSES" cashMovements={cashMovements} onAddCashMovement={handleAddCashMovement} fixedCategories={FIXED_EXPENSE_CATEGORIES} onAddFixedCategory={() => {}} />;
      case ViewState.FIXED_INCOME: return <FinanceManagerModule activeTab="INCOME" cashMovements={cashMovements} onAddCashMovement={handleAddCashMovement} fixedCategories={FIXED_INCOME_CATEGORIES} onAddFixedCategory={() => {}} />;
      case ViewState.CREDIT_NOTE: return <CreditNoteModule salesHistory={salesHistory} bankAccounts={bankAccounts} onProcessCreditNote={(originalSaleId, itemsToReturn, totalRefund, breakdown, detailedRefunds) => { const date = new Date().toLocaleDateString('es-PE'); const time = new Date().toLocaleTimeString('es-PE'); const user = session.user.fullName; const ncId = 'NC-' + Math.random().toString(36).substr(2, 6).toUpperCase(); setProducts(current => current.map(p => { const returnItem = itemsToReturn.find(i => i.itemId === p.id); if (returnItem) { const newStock = p.stock + returnItem.quantity; const move: StockMovement = { id: 'MV-NC-' + Math.random().toString(36).substr(2, 6).toUpperCase(), date, time, productId: p.id, productName: p.name, type: 'ENTRADA', quantity: returnItem.quantity, currentStock: newStock, reference: `NC #${ncId} (DEV. TICKET #${originalSaleId})`, user }; setStockMovements(prev => [move, ...prev]); return { ...p, stock: newStock }; } return p; })); if (detailedRefunds) { detailedRefunds.forEach(ref => handleAddCashMovement({ id: Math.random().toString(), date, time, type: 'Ingreso', paymentMethod: ref.method, concept: `REEMBOLSO NC #${ncId} - DEV. VENTA #${originalSaleId}`, user, category: 'DEVOLUCION', financialType: 'Variable', amount: ref.amount, accountId: ref.accountId })); } alert(`Nota de Crédito ${ncId} procesada.`); }} />;
      ...
      case ViewState.KARDEX_HISTORY: return <HistoryQueries salesHistory={salesHistory} purchasesHistory={purchasesHistory} stockMovements={stockMovements} initialTab="kardex" />;
      case ViewState.HISTORY_QUERIES: return <HistoryQueries salesHistory={salesHistory} purchasesHistory={purchasesHistory} stockMovements={stockMovements} initialTab="ventas" />;
      case ViewState.PURCHASES_HISTORY: return <HistoryQueries salesHistory={salesHistory} purchasesHistory={purchasesHistory} stockMovements={stockMovements} initialTab="compras" />;
      case ViewState.CREDIT_NOTE_HISTORY: return <HistoryQueries salesHistory={salesHistory} purchasesHistory={purchasesHistory} stockMovements={stockMovements} initialTab="notas_credito" />;
      ...
      case ViewState.SYSTEM_DIAGNOSTICS: return <SystemDiagnosticsModule products={products} cashMovements={cashMovements} stockMovements={stockMovements} onAddCashMovement={handleAddCashMovement} />;
      ...
      <div className="flex-1 flex flex-col justify-center">
        <LoginScreen 
          onLogin={u => {
            ...
            setSession({ user: u, businessName: u.companyName, token: 'session_active', baseCurrency: tenant?.baseCurrency || 'PEN' });
            if (u.role === 'SUPER_ADMIN') setCurrentView(ViewState.SUPER_ADMIN_DASHBOARD);
  ...
  // SI HAY SESIÓN: Muestra el ERP normal (SIN FOOTER)
  return (
    <div className={`flex flex-col h-screen w-full bg-[#f8fafc] dark:bg-[#020617] overflow-hidden transition-colors duration-300 font-sans ${isDarkMode ? 'dark' : ''}`}>
      <Layout 
        companyName={session.businessName} companyLogo={companyLogo} navStructure={navStructure} 
        currentView={currentView} onNavigate={v => { setWsContact(undefined); setCurrentView(v); }} 
        isDarkMode={isDarkMode} toggleTheme={() => setIsDarkMode(!isDarkMode)} 
        session={session} onLogout={() => setSession(null)} 
        isSyncEnabled={isSyncEnabled} toggleSyncMode={() => setIsSyncEnabled(!isSyncEnabled)}
      >
